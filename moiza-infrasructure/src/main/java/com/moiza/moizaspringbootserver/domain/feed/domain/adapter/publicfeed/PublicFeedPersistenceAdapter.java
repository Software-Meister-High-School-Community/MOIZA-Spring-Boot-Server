...
public class PublicFeedPersistenceAdapter implements PublicFeedSpi {
    ...
    @Override
    public List<PublicFeed> getPublicFeedsByCategory(String category) {
        return publicFeedRepository.findAllByFeedCategoryCategoryName(category)
                .stream()
                .map(publicFeedMapper::publicFeedEntityToDomain)
                .toList();
    }
}
